...
def run_sequential_chains(review):
    ...
    # Run the sequential chain with the given review
    result = seq_chain.run(review=review)
    return result
...
def main():
    ...
    if user_input and st.button("Process Text"):
        results = run_sequential_chains(user_input)
        if all(key in results for key in ['english_text', 'Arabic_text', 'final_plan']):
            st.text_area("English Text:", results['english_text'], height=150)
            st.text_area("Arabic Translation:", results['Arabic_text'], height=150)
            st.text_area("Summarized Arabic Text:", results['final_plan'], height=150)
            pdf_filename = create_pdf(results['english_text'], results['Arabic_text'], results['final_plan'])
            with open(pdf_filename, "rb") as file:
                st.download_button("Download Text Summary", file, file_name=pdf_filename, mime="application/octet-stream")
        else:
            st.error("One or more outputs were missing in the results. Check the sequential chain setup.")
